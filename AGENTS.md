...
- **NUNCA leer env vars desde QML** — `Qt.platform.environment` es `undefined` en las sesiones qml6 de este proyecto (probe directo, 2026-09-13) y `Qt.environment()` tampoco existe: cualquier lectura de env en QML cae SIEMPRE al fallback, en silencio. Para pasar datos del backend a QML usa el **modulo generado** `qml.auth` (`AuthToken.token`, `AuthToken.agentOverlay`): main.rs lo escribe en `~/.cache/kde-assistant/qml-generated/` al arrancar y lo pasa como el ULTIMO `-I` (el motor prefiere el ultimo -I ante modulos duplicados, eclipsando el fallback vacio de `qml/auth/`). Asi se soluciono el personaje duplicado definitivo (FIX v3): el flag del overlay se hornea en el modulo y la ventana standalone queda oculta desde el frame 1, sin Timer ni race.
